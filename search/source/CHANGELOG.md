...
- Exact note-title matches are always ranked above prefix and content matches.
- Title-prefix matches are always ranked above matches elsewhere.
- Renamed the displayed plugin name to Search while retaining the `omnisearch` identifier for compatibility.
- Added the requested Search defaults and a one-time migration from existing Omnisearch settings.
