...
export default class OmnisearchPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    const loadedSettings = await loadSettings(this)
    this.settings = loadedSettings.settings
    if (loadedSettings.presetApplied) {
      await this.database.clearCache(false)
    }
  ...
  addRibbonButton(): void {
    this.ribbonButton = this.addRibbonIcon('search', 'Search', _evt => {
      new OmnisearchVaultModal(this).open()
    })
  }
