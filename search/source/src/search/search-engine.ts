...
import {
  ...
  getTitleMatchRank,
  TitleMatchRank,
  type TitleMatchRank as TitleMatchRankValue,
} from './title-match-ranking'
...
export class SearchEngine {
  ...
  public async search(
    ...
    const queryStr = query.segmentsToStr()
    const titleMatchRanks = new Map<string, TitleMatchRankValue>()
    ...
    for (const result of results) {
      ...
      // Rank exact titles above prefix matches, and prefix matches above all other results.
      if (queryStr) {
        ...
        if (settings.ignoreDiacritics) {
          title = removeDiacritics(title, settings.ignoreArabicDiacritics)
        }

        const titleMatchRank = getTitleMatchRank(title, queryStr)
        if (titleMatchRank !== TitleMatchRank.None) {
          titleMatchRanks.set(path, titleMatchRank)
          logVerbose(
            `${titleMatchRank === TitleMatchRank.Exact ? 'Exact' : 'Prefix'} title match: ${path}`
          )
          result.score *= 100
        }
      }
    ...
    // Sort by strict title-match level first, then use Omnisearch's score within each level.
    results = results
      .sort((a, b) =>
        compareTitleMatches(
          titleMatchRanks.get(a.id as string) ?? TitleMatchRank.None,
          a.score,
          titleMatchRanks.get(b.id as string) ?? TitleMatchRank.None,
          b.score
        )
      )
      .slice(0, 50)
