...
const COPILOT_BRIDGE_PROMPTER_MARKER = Symbol.for("browser-gpt-obsidian.permission-prompter-bridge");
const NATIVE_GPT_OWNER_ATTRIBUTE = "data-gpt-obsidian-owned";
...
class GptObsidianPlugin extends Plugin {
  ...
  isChatGptWebview(webview) {
    if (this.isNativeGptOwnedWebview(webview)) return false;
    return isChatGptUrl(this.getWebviewUrl(webview));
  }

  isNativeGptOwnedWebview(webview) {
    try {
      return webview?.getAttribute?.(NATIVE_GPT_OWNER_ATTRIBUTE) === "true";
    } catch (_) {
      return false;
    }
  }
  ...
  bridgeBindingFor(webview) {
    if (!this.isChatGptWebview(webview)) return null;
