...
function permissionRegressionRequest(fixture) {
  return {
    ...
    toolCall: {
      ...
      status: "pending",
      rawInput: {
        command: "cp -- /home/mcmarkus/Documents/system-obsidian/obsidian/dev/browser-gpt-obsidian/main.js /home/mcmarkus/Documents/system-obsidian/obsidian/.obsidian/plugins/browser-gpt-obsidian/main.js"
      }
    },
    options: [
      { optionId: "allow_once", kind: "allow_once", name: "Allow Once" },
...
test("Browser GPT Obsidian manifest keeps the donor lineage under its new plugin identity", () => {
  assert.equal(manifest.id, "browser-gpt-obsidian");
  assert.equal(manifest.name, "Browser GPT Obsidian");
...
test("Browser plugin binds Core Web Viewer ChatGPT and ignores explicitly owned native GPT webviews", () => {
  const coreGuest = new FakeGuest(601);
  const ownedGuest = new FakeGuest(602);
  const coreWebview = new FakeWebview(601, true, "https://chatgpt.com/c/core-viewer");
  const ownedWebview = new FakeWebview(602, true, "https://chatgpt.com/c/native-view");
  ownedWebview.attributes.set("data-gpt-obsidian-owned", "true");
  const leaves = [
    { id: "leaf-core-viewer", view: { webview: coreWebview, containerEl: null } },
    { id: "leaf-native-gpt", view: { webview: ownedWebview, containerEl: null } }
  ];
  const plugin = makePlugin(leaves, new Map([[601, coreGuest], [602, ownedGuest]]));
  let bridgeCalls = 0;
  plugin.bridgeBindingFor = () => {
    bridgeCalls += 1;
    return null;
  };
  plugin.applyAppearance = () => true;
  ...
  GptObsidianPlugin.prototype.bindWebview.call(plugin, coreWebview);
  coreWebview.emit("dom-ready");

  assert.equal(plugin.isChatGptWebview(coreWebview), true);
  assert.equal(plugin.boundWebviews.has(coreWebview), true);
  assert.equal(coreGuest.listenerCount("before-input-event"), 1);
  const coreBridgeCalls = bridgeCalls;

  GptObsidianPlugin.prototype.bindWebview.call(plugin, ownedWebview);

  assert.equal(plugin.isChatGptWebview(ownedWebview), false);
  assert.equal(plugin.boundWebviews.has(ownedWebview), false);
  assert.equal(ownedGuest.listenerCount("before-input-event"), 0);
  assert.equal(plugin.guestBindings.has(ownedGuest.id), false);
  assert.equal(bridgeCalls, coreBridgeCalls);
  assert.equal(GptObsidianPlugin.prototype.bridgeBindingFor.call(plugin, ownedWebview), null);
  assert.equal(plugin.bridgeBindings.has(ownedWebview), false);
});
