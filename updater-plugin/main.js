  ...
  async installPreparedPlugin(info) {
    ...
    try {
      ...
      if (wasEnabled && api?.disablePlugin) {
        await api.disablePlugin(oldPluginId);
      }
      ...
      try {
        ...
        await this.safeRefreshPluginManifestCache(newPluginId, remoteManifest);

        if (wasEnabled) {
          if (idChanged && typeof api?.enablePluginAndSave === "function") {
            await api.enablePluginAndSave(newPluginId);
          } else if (api?.enablePlugin) {
            await api.enablePlugin(newPluginId);
          }
        }
        ...
        await this.safeRefreshPluginManifestCache(oldPluginId, local.manifest);

        if (wasEnabled) {
          try {
            if (typeof api?.enablePluginAndSave === "function") await api.enablePluginAndSave(oldPluginId);
            else if (api?.enablePlugin) await api.enablePlugin(oldPluginId);
          } catch {}
        }
