...
var TERM_INTERVAL_REVIEW_ID = "term-interval-review";
var REVIEW_TARGET_TYPE = "term-interval-review";
var normalizeReviewTerm = (value) => String(value != null ? value : "").replaceAll("**", "").replace(/^\s*(?:[-+*]|\d+[.)])\s+/u, "").trim();
var getReviewCardKind = (card) => (card == null ? void 0 : card.kind) === "list" || Array.isArray(card == null ? void 0 : card.listTerms) ? "list" : "definition";
var getTermIntervalReviewPlugin = (app) => {
  var _a, _b;
  return (_b = (_a = app == null ? void 0 : app.plugins) == null ? void 0 : _a.getPlugin) == null ? void 0 : _b.call(_a, TERM_INTERVAL_REVIEW_ID);
};
var getMatchingDefinition = (definitions, sourcePath, term, occurrence) => {
  const normalizedTerm = normalizeReviewTerm(term);
  return definitions.find((card) => card.sourcePath === sourcePath && normalizeReviewTerm(card.term) === normalizedTerm && (occurrence === void 0 || Number(card.occurrence || 0) === Number(occurrence)));
};
var getQualifiedDefinitionIds = (listCard, definitions) => {
  const ids = /* @__PURE__ */ new Set();
  if (Array.isArray(listCard.definitionRefs)) {
    for (const reference of listCard.definitionRefs) {
      if (!reference || typeof reference.term !== "string")
        continue;
      const definition = getMatchingDefinition(definitions, listCard.sourcePath, reference.term, reference.occurrence);
      if (definition)
        ids.add(definition.id);
    }
    return ids;
  }
  for (const term of Array.isArray(listCard.listTerms) ? listCard.listTerms : []) {
    const definition = getMatchingDefinition(definitions, listCard.sourcePath, term);
    if (definition)
      ids.add(definition.id);
  }
  return ids;
};
var getEligibleReviewCards = (cards) => {
  const source = Array.isArray(cards) ? cards : [];
  const definitions = source.filter((card) => getReviewCardKind(card) === "definition" && typeof (card == null ? void 0 : card.id) === "string" && normalizeReviewTerm(card == null ? void 0 : card.term).length > 0 && typeof (card == null ? void 0 : card.sourcePath) === "string");
  const lists = source.filter((card) => getReviewCardKind(card) === "list" && typeof (card == null ? void 0 : card.id) === "string" && normalizeReviewTerm(card == null ? void 0 : card.term).length > 0 && typeof (card == null ? void 0 : card.sourcePath) === "string" && getQualifiedDefinitionIds(card, definitions).size >= 2);
  return [...definitions, ...lists];
};
var getEligibleReviewCardsForSource = (app, sourcePath) => {
  var _a;
  const plugin = getTermIntervalReviewPlugin(app);
  return getEligibleReviewCards((_a = plugin == null ? void 0 : plugin.cards) != null ? _a : []).filter((card) => card.sourcePath === sourcePath);
};
var getReviewIndexSignature = (app) => {
  var _a;
  const plugin = getTermIntervalReviewPlugin(app);
  return JSON.stringify(getEligibleReviewCards((_a = plugin == null ? void 0 : plugin.cards) != null ? _a : []).map((card) => [card.id, getReviewCardKind(card), normalizeReviewTerm(card.term), card.sourcePath]).sort((left, right) => String(left[0]).localeCompare(String(right[0]))));
};
var ReviewTarget = class {
  constructor(app, card) {
    this.targetType = REVIEW_TARGET_TYPE;
    this.app = app;
    this.cardId = card.id;
    this.reviewKind = getReviewCardKind(card);
    this.term = normalizeReviewTerm(card.term);
    this.basename = this.term;
    this.sourcePath = card.sourcePath;
    this.path = card.sourcePath;
  }
  async open() {
    var _a;
    const plugin = getTermIntervalReviewPlugin(this.app);
    if (!plugin || typeof plugin.openCard !== "function") {
      new import_obsidian2.Notice("Включите плагин «Повторение терминов», чтобы открыть карточку.");
      return;
    }
    let card = Array.isArray(plugin.cards) ? plugin.cards.find((candidate) => candidate.id === this.cardId) : null;
    if (!card && typeof plugin.synchronizeFile === "function") {
      const file = (_a = this.app.vault) == null ? void 0 : _a.getAbstractFileByPath(this.sourcePath);
      if (file instanceof import_obsidian2.TFile && file.extension === "md") {
        await plugin.synchronizeFile(file, true, true);
        card = Array.isArray(plugin.cards) ? plugin.cards.find((candidate) => candidate.id === this.cardId) : null;
      }
    }
    if (!card) {
      new import_obsidian2.Notice(`Карточка «${this.term}» больше не найдена.`);
      return;
    }
    await plugin.openCard(card.id);
  }
};
var isReviewTarget = (target) => (target == null ? void 0 : target.targetType) === REVIEW_TARGET_TYPE;
var getTargetPriority = (target) => isReviewTarget(target) ? target.reviewKind === "definition" ? 2 : 1 : 0;
var PrefixTree = class {
  ...
  getCurrentMatchNodes(index, excludedNote) {
    ...
    for (const node of this._currentNodes) {
      ...
      let targets = Array.from(node.node.files).filter((file) => !excludedNote || file.path !== excludedNote.path);
      const highestPriority = targets.reduce((priority, target) => Math.max(priority, getTargetPriority(target)), 0);
      if (highestPriority > 0) {
        targets = targets.filter((target) => getTargetPriority(target) === highestPriority);
      }
      matchNode.files = new Set(targets);
  ...
  addFileToTree(file) {
    ...
    const reviewTargets = getEligibleReviewCardsForSource(this.app, path2).map((card) => new ReviewTarget(this.app, card));
    for (const target of reviewTargets) {
      const name = target.term;
      if (!PrefixTree.isNoneEmptyString(name))
        continue;
      if (this.settings.matchCaseSensitive) {
        if (tags.includes(this.settings.tagToIgnoreCase)) {
          this.addFileWithName(name, target, false);
          this.addFileWithName(name.toLowerCase(), target, false);
        } else {
          this.addFileWithName(name, target, true);
        }
      } else if (tags.includes(this.settings.tagToMatchCase)) {
        this.addFileWithName(name, target, true);
      } else if (PrefixTree.isUpperCaseString(name, this.settings.capitalLetterProportionForAutomaticMatchCase)) {
        this.addFileWithName(name, target, true);
      } else {
        this.addFileWithName(name, target, false);
        this.addFileWithName(name.toLowerCase(), target, false);
...
var VirtualMatch = class {
  ...
  getCompleteLinkElement() {
    ...
    const firstTarget = this.files.length > 0 ? this.files[0] : "";
  ...
  getLinkAnchorElement(linkText, target) {
    ...
    const href = isReviewTarget(target) ? `#virtual-linker-review-${encodeURIComponent(target.cardId)}` : typeof target === "string" ? target : target.path;
    link.href = href;
    ...
    if (isReviewTarget(target)) {
      link.setAttribute("data-href", `virtual-linker-review:${target.cardId}`);
      link.setAttribute("data-virtual-linker-target", "review-card");
      link.setAttribute("data-vl-review-card-id", target.cardId);
      link.setAttribute("data-vl-review-kind", target.reviewKind);
      link.setAttribute("data-vl-review-source", target.sourcePath);
      link.setAttribute("aria-label", `Открыть карточку повторения: ${target.term}`);
      link.setAttribute("draggable", "false");
      link.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        void target.open().catch((error) => console.error("Не удалось открыть карточку повторения из Virtual Linker", error));
      });
    }
    return link;
  }
...
var LinkerPlugin = class extends import_obsidian5.Plugin {
  ...
  async onload() {
    ...
    this.reviewIndexSignature = getReviewIndexSignature(this.app);
    ...
    this.registerInterval(window.setInterval(() => {
      const signature = getReviewIndexSignature(this.app);
      if (signature === this.reviewIndexSignature)
        return;
      this.reviewIndexSignature = signature;
...
LinkerPlugin.__test = {
  ...
  getQualifiedDefinitionIds,
  getReviewCardKind,
  getReviewIndexSignature,
  isPathInDirectories,
  isReviewTarget,
  normalizeReviewTerm
};
