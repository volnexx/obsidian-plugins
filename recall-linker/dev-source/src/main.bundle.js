...
var PRIORITY_RECALL_ID = "term-interval-review";
var JUMP_TO_LINK_ID = "mrj-jump-to-link";
var RECALL_LINK_CLASS = "recall-link-a";
var getCardKind = (card) => card && (card.kind === "list" || Array.isArray(card.listTerms)) ? "list" : "definition";
var normalizeRecallTerm = (term) => String(term != null ? term : "").replaceAll("**", "").replace(/^\s*(?:[-+*]|\d+[.)])\s+/u, "").trim();
var isRecallTarget = (target) => typeof (target == null ? void 0 : target.recallCardId) === "string";
var countFullDefinitionsForList = (listCard, cards) => {
  if (Array.isArray(listCard.definitionRefs))
    return listCard.definitionRefs.length;
  const counts = /* @__PURE__ */ new Map();
  cards.filter((card) => getCardKind(card) === "definition" && card.sourcePath === listCard.sourcePath).forEach((card) => {
    const term = normalizeRecallTerm(card.term);
    counts.set(term, (counts.get(term) || 0) + 1);
  });
  let total = 0;
  for (const item of Array.isArray(listCard.listTerms) ? listCard.listTerms : []) {
    const term = normalizeRecallTerm(item);
    const remaining = counts.get(term) || 0;
    if (remaining <= 0)
      continue;
    counts.set(term, remaining - 1);
    total += 1;
  }
  return total;
};
var getRecallCards = (cards) => (Array.isArray(cards) ? cards : []).filter((card) => {
  if (!card || typeof card.id !== "string" || typeof card.term !== "string" || typeof card.sourcePath !== "string")
    return false;
  return getCardKind(card) === "definition" || countFullDefinitionsForList(card, cards) >= 2;
});
var getPriorityRecall = (app) => {
  var _a, _b;
  return (_b = (_a = app.plugins) == null ? void 0 : _a.getPlugin(PRIORITY_RECALL_ID)) != null ? _b : null;
};
var openRecallCard = async (app, cardId) => {
  const priorityRecall = getPriorityRecall(app);
  if (!priorityRecall || typeof priorityRecall.openCard !== "function") {
    new import_obsidian.Notice("Включите плагин «Повторение терминов», чтобы открыть карточку.");
    return false;
  }
  const card = Array.isArray(priorityRecall.cards) ? priorityRecall.cards.find((candidate) => candidate.id === cardId) : null;
  if (!card) {
    new import_obsidian.Notice("Карточка повторения больше не существует.");
    return false;
  }
  await priorityRecall.openCard(card.id);
  return true;
};
var createRecallTarget = (app, card) => ({
  path: `recall-linker:${card.id}`,
  basename: normalizeRecallTerm(card.term),
  sourcePath: card.sourcePath,
  recallCardId: card.id,
  recallKind: getCardKind(card),
  openCard: () => openRecallCard(app, card.id)
});
var keepRecallTargets = (matches) => matches.map((match) => {
  match.files = new Set(Array.from(match.files).filter(isRecallTarget));
  return match;
}).filter((match) => match.files.size > 0);
...
var LinkerMetaInfoFetcher = class {
  ...
  refreshSettings(settings) {
    ...
    this.excludeDirPattern = createDirectoryPattern(this.settings.excludedDirectories);
  }
  getMetaInfo(file) {
...
var PrefixTree = class {
  constructor(app, settings) {
    ...
    this.recallTargetPaths = /* @__PURE__ */ new Set();
  ...
  getCurrentMatchNodes(index, excludedNote) {
    ...
    for (const node of this._currentNodes) {
      ...
      const candidates = Array.from(node.node.files).filter((file) => !excludedNote || isRecallTarget(file) || file.path !== excludedNote.path);
      const recallTargets = candidates.filter(isRecallTarget);
      matchNode.files = new Set(recallTargets.length > 0 ? recallTargets : candidates);
  ...
  addFileToTree(file) {
    ...
    if (excludeFile || isInExcludedDir) {
      return;
    }
  ...
  removeRecallTargets() {
    for (const targetPath of this.recallTargetPaths) {
      this.removeFileFromTree(targetPath);
    }
    this.recallTargetPaths.clear();
  }
  addRecallCardToTree(card) {
    const sourceFile = this.app.vault.getAbstractFileByPath(card.sourcePath);
    if (!(sourceFile instanceof import_obsidian2.TFile))
      return;
    const metaInfo = this.fetcher.getMetaInfo(sourceFile);
    if (metaInfo.excludeFile || metaInfo.isInExcludedDir)
      return;
    if (!metaInfo.includeFile && !metaInfo.isInIncludedDir && !metaInfo.includeAllFiles)
      return;
    const target = createRecallTarget(this.app, card);
    const term = target.basename;
    if (!PrefixTree.isNoneEmptyString(term))
      return;
    const tags = ((0, import_obsidian2.getAllTags)(this.app.metadataCache.getFileCache(sourceFile)) || []).filter(PrefixTree.isNoneEmptyString).map((tag) => tag.startsWith("#") ? tag.slice(1) : tag);
    let requiresCaseMatch = this.settings.matchCaseSensitive;
    if (this.settings.matchCaseSensitive && tags.includes(this.settings.tagToIgnoreCase)) {
      requiresCaseMatch = false;
    } else if (!this.settings.matchCaseSensitive) {
      requiresCaseMatch = tags.includes(this.settings.tagToMatchCase) || PrefixTree.isUpperCaseString(term, this.settings.capitalLetterProportionForAutomaticMatchCase);
    }
    this.addFileWithName(term, target, requiresCaseMatch);
    if (!requiresCaseMatch)
      this.addFileWithName(term.toLowerCase(), target, false);
    this.recallTargetPaths.add(target.path);
  }
  refreshRecallTargets() {
    this.removeRecallTargets();
    const priorityRecall = getPriorityRecall(this.app);
    const cards = getRecallCards(priorityRecall == null ? void 0 : priorityRecall.cards);
    cards.forEach((card) => this.addRecallCardToTree(card));
  }
  fileIsUpToDate(file) {
...
var VirtualMatch = class {
  ...
  getCompleteLinkElement() {
    ...
    const firstTarget = this.files.length > 0 ? this.files[0] : null;
    if (!this.isSubWord || !this.settings.suppressSuffixForSubWords) {
      const icon = this.getIconSpan(firstTarget);
      if (icon)
  ...
  getLinkAnchorElement(linkText, target) {
    const link = document.createElement("a");
    link.href = isRecallTarget(target) ? `#recall-card-${encodeURIComponent(target.recallCardId)}` : (target == null ? void 0 : target.path) || "";
    ...
    if (isRecallTarget(target)) {
      link.classList.add(RECALL_LINK_CLASS);
      link.dataset.recallCardId = target.recallCardId;
      link.dataset.href = `recall-card:${target.recallCardId}`;
      link.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
        void target.openCard();
      });
    }
    return link;
  }
  ...
  getMultipleReferencesSpan(files) {
    ...
    files.forEach((file, index) => {
      ...
      const link = this.getLinkAnchorElement(linkText, file);
  ...
  getIconSpan(target) {
    ...
    if (((_a = suffix == null ? void 0 : suffix.length) != null ? _a : 0) > 0) {
      ...
      icon.appendChild(this.getLinkAnchorElement(suffix, target));
      return icon;
    }
    return null;
  }
...
var GlossaryLinker = class extends import_obsidian3.MarkdownRenderChild {
  ...
  onload() {
    ...
    const tags = ["p", "li", "td", "th", "span", "em", "strong", "h1", "h2", "h3", "h4", "h5", "h6"];
      ...
      for (let index = 0; index <= nodeList.length; index++) {
        ...
        for (let childNodeIndex = 0; childNodeIndex < item.childNodes.length; childNodeIndex++) {
          ...
          if (childNode.nodeType === Node.TEXT_NODE) {
            ...
            const isHeader = /^H[1-6]$/u.test(item.tagName || "");
            if (isHeader && !this.settings.includeHeaders)
              matches = keepRecallTargets(matches);
...
var AutoLinkerPlugin = class {
  ...
  buildDecorations(view, viewIsActive = true) {
    ...
    for (let { from, to } of view.visibleRanges) {
      ...
      const headerIntervalTree = new IntervalTree();
      ...
      (0, import_language.syntaxTree)(view.state).iterate({
        ...
        enter(node) {
          ...
          if (!settings.includeHeaders && type.includes("header-")) {
            headerIntervalTree.insert([node.from, node.to]);
          }
      ...
      if (!this.settings.includeHeaders) {
        matches = matches.map((match) => {
          if (headerIntervalTree.search([match.from, match.to]).length > 0) {
            match.files = new Set(Array.from(match.files).filter(isRecallTarget));
          }
          return match;
        }).filter((match) => match.files.size > 0);
...
var getHintLetters = (alphabet, count) => {
  ...
  for (const prefix of prefixes) {
    for (const character of characters) {
      if (result.length >= count)
        return result;
      if (prefix === "" && reserved.has(character))
        continue;
...
var DEFAULT_SETTINGS = {
  ...
  defaultLinkFormat: "absolute",
  useDefaultLinkStyleForConversion: true,
  applyDefaultLinkStyling: false,
  ...
  showAllLongestMatchesMigration: 1
};
var LinkerPlugin = class extends import_obsidian5.Plugin {
  ...
  async onload() {
    ...
    this.registerInterval(window.setInterval(() => this.patchJumpToLink(), 2e3));
    for (const eventName of ["create", "modify", "delete", "rename"]) {
      this.registerEvent(this.app.vault.on(eventName, () => this.scheduleIndexRefresh()));
    }
    this.register(() => {
    ...
    this.addCommand({
      id: "activate-recall-linker",
      name: "Включить ссылки повторения",
    ...
    this.addCommand({
      id: "deactivate-recall-linker",
      name: "Выключить ссылки повторения",
      checkCallback: (checking) => {
        if (this.settings.linkerActivated) {
          if (!checking) {
    ...
    this.addCommand({
      id: "convert-selected-recall-links",
  ...
  getRecallSourceModeHints(jumpPlugin) {
    const editor = jumpPlugin.cmEditor;
    const root = editor == null ? void 0 : editor.dom;
    if (!root || typeof root.querySelectorAll !== "function")
      return [];
    const hints = [];
    const seen = /* @__PURE__ */ new Set();
    for (const anchor of root.querySelectorAll(`a.${RECALL_LINK_CLASS}[data-recall-card-id]`)) {
      const cardId = anchor.dataset.recallCardId;
      const index = Number(anchor.getAttribute("to"));
      if (!cardId || !Number.isFinite(index))
        continue;
      ...
      hints.push({
        index,
        type: "recall-link",
        linkText: anchor.getAttribute("origin-text") || "",
        recallCardId: cardId,
        letter: ""
      });
    }
  ...
  patchJumpToLink() {
    var _a, _b;
    const jumpPlugin = (_b = (_a = this.app.plugins) == null ? void 0 : _a.getPlugin(JUMP_TO_LINK_ID)) != null ? _b : null;
    if (!jumpPlugin || typeof jumpPlugin.handleActions !== "function" || typeof jumpPlugin.handleHotkey !== "function") {
      if (this.jumpPatch && this.jumpPatch.plugin !== jumpPlugin)
        this.restoreJumpToLink();
      return;
    }
    if ((this.jumpPatch == null ? void 0 : this.jumpPatch.plugin) === jumpPlugin)
      return;
    this.restoreJumpToLink();
    ...
    function patchedHandleHotkey(heldShiftKey, link) {
      if ((link == null ? void 0 : link.type) === "recall-link" && link.recallCardId) {
        void openRecallCard(bridge.app, link.recallCardId);
        return;
      }
    ...
    function patchedHandleActions(linkHints, linkHintHtmlElements) {
      if (this.mode !== 0)
        return originalHandleActions.call(this, linkHints, linkHintHtmlElements);
      const recallHints = bridge.getRecallSourceModeHints(this);
      if (recallHints.length === 0)
        return originalHandleActions.call(this, linkHints, linkHintHtmlElements);
      const combined = [...linkHints, ...recallHints].sort((left, right) => (left.index || 0) - (right.index || 0));
      const letters = getHintLetters(this.settings == null ? void 0 : this.settings.letters, combined.length);
      combined.forEach((hint, index) => {
        hint.letter = letters[index] || "";
      });
      const visible = combined.filter((hint) => hint.letter);
      try {
        const marker = this.cmEditor && this.cmEditor.plugin ? this.cmEditor.plugin(this.markViewPlugin) : null;
        if (marker == null ? void 0 : marker.setLinks)
          marker.setLinks(visible);
        bridge.app.workspace.updateOptions();
      } catch (error) {
        console.error("Не удалось добавить подсказки ссылок повторения в Jump to Link", error);
      }
  ...
  restoreJumpToLink() {
    const patch2 = this.jumpPatch;
    if (!patch2)
      return;
    if (patch2.plugin.handleActions === patch2.patchedHandleActions)
      patch2.plugin.handleActions = patch2.originalHandleActions;
    if (patch2.plugin.handleHotkey === patch2.patchedHandleHotkey)
      patch2.plugin.handleHotkey = patch2.originalHandleHotkey;
    this.jumpPatch = null;
  }
  ...
  async loadSettings() {
    ...
    if (!savedSettings) {
      try {
        const legacyContent = await this.app.vault.adapter.read(this.app.vault.configDir + "/plugins/virtual-linker/data.json");
    ...
    const firstInstall = !savedSettings;
    ...
    if (firstInstall || !await this.loadData())
      await this.saveData(this.settings);
